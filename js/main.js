...
diamondcost = Math.floor(Math.pow(1.7,(bonus["reespeccost"]))*5)
if(prestige["diamond"]<treasurecost){
	$(".reespec2").addClass("unavailable")
}
else
{
	$(".reespec2").removeClass("unavailable")
}
$(".reespec2").html("Respec");
$(".reespec2").attr('tooltip', 'Diamonds: '+ parseFloat(craft["diamond"]).toFixed(2)+" / "+parseFloat(diamondcost).toFixed(2))
$(".reespec2").attr('tooltip3', 'Lets you get your titles back, for a cost..')
...
tradecopper=Math.pow(1.5,(buildings["market"]))*15
tradetin=Math.pow(1.5,(buildings["market"]))*10
tradecoal=Math.pow(1.5,(buildings["market"]))*10
tradeiron=Math.pow(1.5,(buildings["market"]))*5
tradegold=Math.pow(1.5,(buildings["market"]))*1
if(craft["coin"]<1){
...
if(craft["diamond"]<1){
	$(".trade_copper").addClass("unavailable")
	$(".trade_tin").addClass("unavailable")
	$(".trade_iron").addClass("unavailable")
	$(".trade_coal").addClass("unavailable")
	$(".trade_gold").addClass("unavailable")
}
else
{

	$(".trade_copper").removeClass("unavailable")
	$(".trade_tin").removeClass("unavailable")
	$(".trade_iron").removeClass("unavailable")
	$(".trade_coal").removeClass("unavailable")
	$(".trade_gold").removeClass("unavailable")
}
...
$(".trade_copper").html("Copper: " + Math.round(tradecopper));
$(".trade_tin").html("Tin: " + Math.round(tradetin));
$(".trade_iron").html("Iron: " + Math.round(tradeiron));
$(".trade_coal").html("Coal: " + Math.round(tradecoal));
$(".trade_gold").html("Gold: " + Math.round(tradegold));
}
...
function dtrade(b){
	...
	if (craft["diamond"]>=1){
		...
		if(b=="copper"){
			items["copper"]+=Math.round(tradecopper)
			craft["diamond"]-=1
		}
		else if(b=="tin"){
			items["tin"]+=Math.round(tradetin)
			craft["diamond"]-=1
		}
		else if(b=="iron"){
			items["iron"]+=Math.round(tradeiron)
			craft["diamond"]-=1
		}
		else if(b=="coal"){
			items["coal"]+=Math.round(tradecoal)
			craft["diamond"]-=1
		}
		else if(b=="gold"){
			items["sand"]+=Math.round(tradegold)
			craft["diamond"]-=1
		}
	}
